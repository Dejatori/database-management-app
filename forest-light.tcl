...
namespace eval ttk::theme::forest-light {
    ...
    ttk::style theme create forest-light -parent default -settings {
        ...
        # Custom CreateButton layout
        ttk::style layout CreateButton {
            CreateButton.button -children {
                CreateButton.padding -children {
                    CreateButton.label -side left -expand true
                }
            }
        }

        # Custom UpdateButton layout
        ttk::style layout UpdateButton {
            UpdateButton.button -children {
                UpdateButton.padding -children {
                    UpdateButton.label -side left -expand true
                }
            }
        }

        # Custom DeleteButton layout
        ttk::style layout DeleteButton {
            DeleteButton.button -children {
                DeleteButton.padding -children {
                    DeleteButton.label -side left -expand true
                }
            }
        }

        # Custom RefreshButton layout
        ttk::style layout RefreshButton {
            RefreshButton.button -children {
                RefreshButton.padding -children {
                    RefreshButton.label -side left -expand true
                }
            }
        }

        # Custom FilterButton layout
        ttk::style layout FilterButton {
            FilterButton.button -children {
                FilterButton.padding -children {
                    FilterButton.label -side left -expand true
                }
            }
        }

        # Custom ClearFilterButton layout
        ttk::style layout ClearFilterButton {
            ClearFilterButton.button -children {
                ClearFilterButton.padding -children {
                    ClearFilterButton.label -side left -expand true
                }
            }
        }

        # Custom PreviousButton layout
        ttk::style layout PreviousButton {
            PreviousButton.button -children {
                PreviousButton.padding -children {
                    PreviousButton.label -side left -expand true
                }
            }
        }

        # Custom NextButton layout
        ttk::style layout NextButton {
            NextButton.button -children {
                NextButton.padding -children {
                    NextButton.label -side left -expand true
                }
            }
        }
        ...
        # Custom CreateButton
        ttk::style configure CreateButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create CreateButton.button image \
            [list $I(create-btn) \
            	{selected disabled} $I(create-btn-hover) \
                disabled $I(create-btn-hover) \
                selected $I(create-btn) \
                pressed $I(create-btn) \
                active $I(create-btn-hover) \
            ] -border 4 -sticky nsew

        # Custom UpdateButton
        ttk::style configure UpdateButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create UpdateButton.button image \
            [list $I(update-btn) \
            	{selected disabled} $I(update-btn-hover) \
                disabled $I(update-btn-hover) \
                selected $I(update-btn) \
                pressed $I(update-btn) \
                active $I(update-btn-hover) \
            ] -border 4 -sticky nsew

        # Custom DeleteButton
        ttk::style configure DeleteButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create DeleteButton.button image \
            [list $I(delete-btn) \
                {selected disabled} $I(delete-btn-hover) \
                disabled $I(delete-btn-hover) \
                selected $I(delete-btn) \
                pressed $I(delete-btn) \
                active $I(delete-btn-hover) \
            ] -border 4 -sticky nsew

        # Custom RefreshButton
        ttk::style configure RefreshButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create RefreshButton.button image \
            [list $I(refresh-btn) \
                {selected disabled} $I(refresh-btn-hover) \
                disabled $I(refresh-btn-hover) \
                selected $I(refresh-btn) \
                pressed $I(refresh-btn) \
                active $I(refresh-btn-hover) \
            ] -border 4 -sticky nsew

        # Custom FilterButton
        ttk::style configure FilterButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create FilterButton.button image \
            [list $I(filter-btn) \
                {selected disabled} $I(filter-btn-hover) \
                disabled $I(filter-btn-hover) \
                selected $I(filter-btn) \
                pressed $I(filter-btn) \
                active $I(filter-btn-hover) \
            ] -border 4 -sticky nsew

        # Custom ClearFilterButton
        ttk::style configure ClearFilterButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create ClearFilterButton.button image \
            [list $I(clear-filter-btn) \
                {selected disabled} $I(clear-filter-btn-hover) \
                disabled $I(clear-filter-btn-hover) \
                selected $I(clear-filter-btn) \
                pressed $I(clear-filter-btn) \
                active $I(clear-filter-btn-hover) \
            ] -border 4 -sticky nsew

        # Custom PreviousButton
        ttk::style configure PreviousButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create PreviousButton.button image \
            [list $I(previous-btn) \
                {selected disabled} $I(previous-btn-hover) \
                disabled $I(previous-btn-hover) \
                selected $I(previous-btn) \
                pressed $I(previous-btn) \
                active $I(previous-btn-hover) \
            ] -border 4 -sticky nsew

        # Custom NextButton
        ttk::style configure NextButton -padding {8 4 8 4} -width -10 -anchor center -foreground #eeeeee

        ttk::style element create NextButton.button image \
            [list $I(next-btn) \
                {selected disabled} $I(next-btn-hover) \
                disabled $I(next-btn-hover) \
                selected $I(next-btn) \
                pressed $I(next-btn) \
                active $I(next-btn-hover) \
            ] -border 4 -sticky nsew
